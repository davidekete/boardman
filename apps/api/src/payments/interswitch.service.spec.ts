...
describe('InterswitchService', () => {
  ...
  describe('initiatePayment', () => {
    const tokenResponse = { data: { access_token: 'tok-abc', expires_in: 3600 } };
    const payResponse = { data: { paymentUrl: 'https://pay.interswitch.com/pay', reference: 'REF123' } };

    it('fetches an access token and returns paymentUrl + reference', async () => {
      http.post.mockReturnValueOnce(of(tokenResponse)).mockReturnValueOnce(of(payResponse));

      const result = await service.initiatePayment({ amount: 50000, email: 'user@example.com', userId: 'u1' });

      expect(result).toEqual({ paymentUrl: 'https://pay.interswitch.com/pay', reference: 'REF123' });
    });

    it('sends the amount as a string and includes merchant config in the request body', async () => {
      http.post.mockReturnValueOnce(of(tokenResponse)).mockReturnValueOnce(of(payResponse));

      await service.initiatePayment({ amount: 50000, email: 'user@example.com', userId: 'u1' });
    ...
    it('authorizes the token request with Basic credentials', async () => {
      ...
      await service.initiatePayment({ amount: 100, email: 'a@b.com', userId: 'u1' });

      const [, , tokenConfig] = http.post.mock.calls[0];
      const expectedCredentials = Buffer.from('client123:secret456').toString('base64');
      expect(tokenConfig.headers.Authorization).toBe(`Basic ${expectedCredentials}`);
    });
    ...
    it('caches the access token — only one OAuth call across two initiations', async () => {
      ...
      await service.initiatePayment({ amount: 100, email: 'a@b.com', userId: 'u1' });
      await service.initiatePayment({ amount: 200, email: 'a@b.com', userId: 'u1' });
    ...
    it('re-fetches the token after the cached one expires (60s buffer)', async () => {
      ...
      http.post
        .mockReturnValueOnce(of({ data: { access_token: 'tok-1', expires_in: 61 } }))
        .mockReturnValueOnce(of(payResponse))
        .mockReturnValueOnce(of({ data: { access_token: 'tok-2', expires_in: 3600 } }))
        .mockReturnValueOnce(of(payResponse));
  ...
  describe('verifyPayment', () => {
    it('returns { success: true, amount } when ResponseCode is "00"', async () => {
      http.get.mockReturnValueOnce(of({ data: { ResponseCode: '00', Amount: 50000 } }));
    ...
    it('returns { success: false } when the HTTP call throws', async () => {
      http.get.mockReturnValueOnce(throwError(() => new Error('network error')));
    ...
    it('sends the correct SHA-512 MAC hash in the Hash header', async () => {
      http.get.mockReturnValueOnce(of({ data: { ResponseCode: '00', Amount: 100 } }));
    ...
    it('includes merchantcode, transactionreference and amount in the query string', async () => {
      http.get.mockReturnValueOnce(of({ data: { ResponseCode: '00', Amount: 200 } }));
